...
from components.widgets.file_browser import FileBroswer
from components.bars.left_tab import LeftTab
from components.views.explorer_view import ExplorerView
...
class SidePanel(VerticalScroll):
    ...
    async def on_search_view_symbol_selected(self, event: SearchView.SymbolSelected):
        """
        Handle when symbol is selected from search results
            1. Switch back to explorer view
            2. Navigate to the file in the tree
            3. Open file in code viewer and jump to line
        """
        self.switch_to_explorer()
        ...
        await self.navigate_to_file(event.file_path)
